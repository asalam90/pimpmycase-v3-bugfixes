...
from backend.routes.stickers import router as stickers_router
...
from backend.routes.chinese_image_preview import router as chinese_image_preview_router
...
# AI service imports moved to routes/image.py - no longer needed here
...
app.include_router(chinese_image_preview_router)
...
if os.path.exists("dist"):
    ...
    if os.path.exists("dist/ui-mockups"):
        app.mount("/ui-mockups", StaticFiles(directory="dist/ui-mockups"), name="ui-mockups")

    # Mount image directories (with spaces in names - browsers will URL-encode them)
    if os.path.exists("dist/Phone backs"):
        app.mount("/Phone backs", StaticFiles(directory="dist/Phone backs"), name="phone-backs")
    if os.path.exists("dist/masks"):
        app.mount("/masks", StaticFiles(directory="dist/masks"), name="masks")
    if os.path.exists("dist/Backgrounds"):
        app.mount("/Backgrounds", StaticFiles(directory="dist/Backgrounds"), name="backgrounds")
    if os.path.exists("dist/Stickers"):
        app.mount("/Stickers", StaticFiles(directory="dist/Stickers"), name="stickers")
    ...
    async def serve_static_files(filename: str):
        ...
        # CRITICAL: Don't serve SPA for API routes - let them return 404 if not found
