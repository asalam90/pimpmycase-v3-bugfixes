...
const RetroRemixScreen = () => {
  ...
  const [availableKeywords, setAvailableKeywords] = useState(defaultRetroKeywords)

  // AI Credits from centralized state
  const aiCredits = appState.aiCredits

  // Fetch available keywords from API
  useEffect(() => {
    const fetchKeywords = async () => {
      try {
        ...
        console.error('Failed to fetch Retro Remix keywords, using defaults', err)
      }
    }
    fetchKeywords()
  }, [])
  ...
  const handleGenerate = async () => {
    if (appState.aiCredits <= 0 || !originalImageFile || !keyword) return
    ...
    try {
      ...
      const result = await aiImageService.generateRetroRemix(keyword, '', originalImageFile, 'low')
    ...
    <div className="screen-container" style={{ backgroundColor: '#fdfdfd' }}>
      ...
      <div className="relative z-10 flex flex-col items-center px-6 mt-2">
        ...
        <div className="flex items-center justify-center p-4 w-full">
          <h1 style={{ fontFamily: '"SF Pro Display", -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Helvetica, sans-serif', fontWeight: 400, color: '#5d5d5d', fontSize: '24px', textTransform: 'uppercase', letterSpacing: '0' }}>Retro Remix</h1>
        </div>
        ...
        <div className="w-full flex flex-col items-center mb-3">
          <div className="flex items-center w-72 justify-center mx-auto">
            ...
            >
              <option value="" disabled>Select Retro Style</option>
              {availableKeywords.map((style) => (
                <option key={style} value={style}>{style}</option>
              ))}
            </select>
            <button
              onClick={generatedImage ? handleRegenerateClick : handleGenerate}
              disabled={!keyword || appState.aiCredits <= 0 || !originalImageFile}
              className={`flex-shrink-0 w-10 h-10 rounded-2xl border border-gray-300 flex items-center justify-center active:scale-95 transition-transform ml-2 ${
                keyword && appState.aiCredits > 0 && originalImageFile ? 'bg-white cursor-pointer' : 'bg-gray-100 cursor-not-allowed'
              }`}
            >
              <RefreshCw size={18} className={`${keyword && appState.aiCredits > 0 && originalImageFile ? 'text-gray-700' : 'text-gray-400'}`} />
            </button>
          </div>
        </div>
        ...
        {!generatedImage && (
          <div className="w-full max-w-xs mb-4 flex justify-center">
            <button
              ...
              disabled={appState.aiCredits === 0 || isGenerating || !keyword || !originalImageFile}
              style={{
                width: '200px',
                padding: '8px 22px',
                ...
                borderRadius: '100px',
                cursor: appState.aiCredits === 0 || isGenerating || !keyword || !originalImageFile ? 'not-allowed' : 'pointer',
                transition: 'all 200ms ease-out',
                display: 'flex',
                alignItems: 'center',
                justifyContent: 'center',
                opacity: appState.aiCredits === 0 || isGenerating || !keyword || !originalImageFile ? 0.5 : 1
              }}
              onMouseEnter={(e) => {
                if (appState.aiCredits > 0 && !isGenerating && keyword && originalImageFile) {
