  ...
  useEffect(() => {
    ...
    const pollPaymentStatus = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/vending/session/${sessionId}/status`, {
          method: 'GET',
          headers: {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (paymentData?.third_id) {
            ...
            const chineseResponse = await fetch(`${API_BASE_URL}/api/chinese/order/payStatus`, {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
