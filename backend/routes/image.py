...
from backend.services.ai_service import generate_image_nano_banana_pro
...
from backend.config.settings import JWT_SECRET_KEY, API_BASE_URL
...
async def generate_image(
    ...
    size: str = Form("1024x1536"),  # Portrait orientation for phone cases
    ...
    try:
        ...
        response = await generate_image_nano_banana_pro(
            prompt=prompt,
        ...
        if not response.get("success"):
            error_msg = response.get("error", "No image generated")
            raise HTTPException(status_code=500, detail=error_msg)

        # Extract and save image from Nano Banana Pro response
        ...
        # Generate secure URL with token for image access
        public_url = get_image_public_url(filename, partner_type="end_user", expiry_hours=24)
        print(f"🔄 API - Generated secure URL: {public_url}")
...
async def upload_final_composed_image(
    ...
    try:
        ...
        print(f"🔍 Extracted base64 data:")
        print(f"   - Original data URL length: {len(final_image_data)} chars")
        print(f"   - Base64 data length: {len(base64_data)} chars")
        print(f"   - Has comma separator: {',' in final_image_data}")
...
async def refresh_image_token(
    ...
    try:
        ...
        new_secure_url = f"{API_BASE_URL}/image/{filename}?token={new_token}"
