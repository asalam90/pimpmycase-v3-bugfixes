...
from backend.services.ai_service import get_google_client
import os
...
async def health_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        google_ai_status = "healthy"
        google_ai_error = None

        if not api_key:
            google_ai_status = "unhealthy"
            google_ai_error = "Google API key not found in environment variables"
        elif api_key == "your-api-key-here":
            google_ai_status = "unhealthy"
            google_ai_error = "Please replace the placeholder API key with your actual Google API key"
        elif not api_key.startswith('AIza'):
            google_ai_status = "unhealthy"
            google_ai_error = "Invalid API key format - should start with 'AIza'"

        # Test basic Google client initialization (without making API calls)
        try:
            if google_ai_status == "healthy":
                client = get_google_client()
                # Just check if client can be created without making network requests
        except Exception as client_error:
            google_ai_status = "unhealthy"
            google_ai_error = f"Google AI client initialization failed: {str(client_error)}"
        ...
        # Overall status
        overall_status = "healthy" if (google_ai_status == "healthy" and database_status == "healthy") else "unhealthy"
        ...
        return {
            ...
            "message": "API ready for image generation with Nano Banana Pro and database backend"
        }
