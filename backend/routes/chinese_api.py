...
async def receive_payment_status_update(
    ...
    try:
        ...
        if vending_session and request.status == 3:  # Payment successful
            ...
            if final_image_url and '/image/' in final_image_url:
                try:
                ...
                elif '/image/' not in final_image_url:
        ...
        if request.status == 3:  # Paid
            ...
            try:
                if order.images:  # If order has images, send to print
                    image_urls = [f"{API_BASE_URL}/image/{img.image_path.split('/')[-1]}"
        ...
        if order.images:
            for img in order.images:
                if img.image_path:
                    ...
                    image_url = f"{API_BASE_URL}/image/{filename}"
        ...
        for img in order.images:
            if img.image_path:
                ...
                download_links.append({
                    ...
                    "secure_url": f"{API_BASE_URL}/image/{filename}?token={secure_token}",
