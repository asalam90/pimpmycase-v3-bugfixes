...
class PhoneModel(Base):
    ...
    chinese_model_id = Column(String(100))  # ARCHIVED: Chinese manufacturer's internal model ID (kept for legacy)
    price = Column(DECIMAL(10, 2), nullable=False, default=35.00)
    stock = Column(Integer, default=999999)  # E-COMMERCE: Unlimited stock (999999 = effectively unlimited)
    is_available = Column(Boolean, default=True)  # Manual control only - admin can disable specific models
    display_order = Column(Integer, default=0)
...
class Order(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    session_id = Column(String(100))  # Web session ID (repurposed from QR session)
    order_number = Column(String(50), unique=True)  # Human-readable order number (ORD-2025-0001)

    # Customer Information (E-Commerce Fields)
    customer_name = Column(String(200))  # Full name
    customer_email = Column(String(200))  # Email address
    customer_phone = Column(String(50))  # Phone number
    shipping_address_line1 = Column(String(500))  # Address line 1
    shipping_address_line2 = Column(String(500))  # Address line 2 (optional)
    shipping_city = Column(String(200))  # City
    shipping_postcode = Column(String(20))  # Postcode/ZIP
    shipping_country = Column(String(100), default="United Kingdom")  # Country
    customer_notes = Column(Text)  # Optional customer notes

    # Legacy user data (JSON fallback)
    user_data = Column(JSON)  # Store any additional user preferences/data
    ...
    queue_number = Column(String(50))  # Queue number for order tracking (repurposed from vending)

    # ARCHIVED: Chinese payment tracking (kept for historical data)
    ...
    chinese_payment_status = Column(Integer)  # 1=waiting, 2=paying, 3=paid, 4=failed, 5=abnormal
    ...
    # ARCHIVED: Vending machine info (kept for historical data, nullable)
...
class PaymentMapping(Base):
    """Mapping between frontend third_id and Chinese API payment IDs"""
    __tablename__ = "payment_mappings"

    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    third_id = Column(String(200), unique=True, nullable=False)  # PYEN format ID
    chinese_payment_id = Column(String(200))  # Chinese API payment transaction ID
    device_id = Column(String(100))  # Vending machine device ID
    mobile_model_id = Column(String(100))  # Chinese model ID
    pay_amount = Column(DECIMAL(10, 2))
    pay_type = Column(Integer)  # 5=vending, 12=stripe
    created_at = Column(DateTime(timezone=True), server_default=func.now())
