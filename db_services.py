...
class PhoneModelService:
    ...
    def get_models_by_brand(db: Session, brand_id: str, include_unavailable: bool = False) -> List[PhoneModel]:
        """Get all models for a specific brand (E-COMMERCE: Stock is unlimited, only filter by is_available)"""
        query = db.query(PhoneModel).filter(PhoneModel.brand_id == brand_id)
        if not include_unavailable:
            # E-COMMERCE MODE: Only filter by manual availability flag, ignore stock
    ...
    def get_model_by_name(db: Session, name: str, brand_id: str = None) -> Optional[PhoneModel]:
        ...
        # Return None if name is None or empty
        if not name:
            return None
    ...
    def update_stock(db: Session, model_id: str, new_stock: int) -> Optional[PhoneModel]:
        """Update model stock (E-COMMERCE: Deprecated - stock is unlimited, but kept for compatibility)"""
        # E-COMMERCE MODE: This method is deprecated but kept for backward compatibility
        # Stock should always remain at 999999 (unlimited)
        # To disable a model, use is_available flag instead
        model = db.query(PhoneModel).filter(PhoneModel.id == model_id).first()
        if model:
            # Warn if trying to set stock below unlimited threshold
            if new_stock < 999999:
                print(f"⚠️  Warning: Attempting to set stock to {new_stock} for {model.name}. E-commerce mode uses unlimited stock.")
                print(f"    To disable this model, set is_available=False instead.")
    ...
    def create_model(db: Session, model_data: Dict[str, Any]) -> PhoneModel:
        """Create new phone model (E-COMMERCE: Stock defaults to unlimited)"""
        model = PhoneModel(
            ...
            stock=model_data.get("stock", 999999),  # E-COMMERCE: Default to unlimited stock
...
class OrderService:
    ...
    def generate_order_number(db: Session) -> str:
        """Generate unique human-readable order number in format ORD-YYYYMMDD-XXXX"""
        from datetime import datetime

        # Get current date in YYYYMMDD format
        date_str = datetime.now().strftime('%Y%m%d')

        # Count orders for today to generate sequence number
        ...
        # Format: ORD-20250116-0001
        order_number = f"ORD-{date_str}-{sequence}"
        ...
        if existing:
            ...
            import random
            order_number = f"{order_number}-{random.randint(100, 999)}"
    ...
    def create_order(db: Session, order_data: Dict[str, Any]) -> Order:
        ...
        if 'order_number' not in order_data or not order_data['order_number']:
            order_data['order_number'] = OrderService.generate_order_number(db)

        # Validate required customer fields for e-commerce orders
        if not order_data.get('customer_name'):
            # If customer info not provided, try to get from user_data JSON (legacy fallback)
            user_data = order_data.get('user_data', {})
            if isinstance(user_data, dict):
                order_data['customer_name'] = user_data.get('name', 'Guest Customer')
                order_data['customer_email'] = user_data.get('email')
                order_data['customer_phone'] = user_data.get('phone')
                order_data['shipping_address_line1'] = user_data.get('address_line1')
                order_data['shipping_address_line2'] = user_data.get('address_line2')
                order_data['shipping_city'] = user_data.get('city')
                order_data['shipping_postcode'] = user_data.get('postcode')
                order_data['shipping_country'] = user_data.get('country', 'United Kingdom')
